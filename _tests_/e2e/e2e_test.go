...
const testText = `Mention is made in many alchemical writings of a mythical personage named Hermes
...
func TestStats(t *testing.T) {
	test.Post("/stats").
		Body(strings.NewReader(testText)).
		Expect(t).
		Status(200).
		Type("json").
		JSON(map[string]interface{}{
			"stats": map[string]interface{}{
				"characters":  1273,
				"lines":       22,
				"top_5_words": []string{"the", "of", "and", "things", "to"},
				"words":       247,
			}},
		).
		Done()
